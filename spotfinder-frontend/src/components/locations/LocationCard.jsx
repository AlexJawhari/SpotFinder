...
const LocationCard = ({ location, distance, isFavorited, onFavoriteToggle, onCenterOnMap }) => {
    ...
    return (
        <div className="bg-white/95 backdrop-blur-md rounded-2xl shadow-xl hover:shadow-2xl transition-all overflow-hidden border-2 border-white/60 hover:scale-[1.02] hover:border-[#7DD3FC]/40">
            <div className="relative">
                ...
                {onCenterOnMap && location.latitude && location.longitude && (
                    <button
                        ...
                        className="absolute bottom-3 left-3 bg-white/95 backdrop-blur-md px-3 py-2 rounded-full shadow-xl hover:shadow-2xl transition-all border border-white/60 hover:scale-[1.02] flex items-center gap-2 text-sm font-semibold text-slate-800"
                ...
                {onFavoriteToggle && (
                    <button
                        ...
                        className="absolute top-3 right-3 bg-white/95 backdrop-blur-md p-2.5 rounded-full shadow-xl hover:scale-110 transition-all hover:bg-white border border-white/50"
                    >
                        {isFavorited ? (
            ...
            <Link to={`/location/${location.id}`} className="block p-5">
                <div className="flex justify-between items-start mb-3">
                    <h3 className="text-xl font-bold text-slate-900 hover:text-[#4FC3F7] transition-all leading-tight">
                        {location.name}
                    </h3>
